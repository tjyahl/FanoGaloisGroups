...
singSoln = apply(n,i->I^(random(4))*(random(10,100)*random(10)/10+I*random(10,100)*random(10)/10));
...
xLS = solve(CCmap(A),transpose matrix {toList(140:(1 + 0*ii))},ClosestFit=>true,MaximalRank=>true);
A = flatten entries transpose ((sub(gens kernel (K||D0),GR))*transpose matrix{apply(flatten entries xLS,z->lift(realPart z,QQ) + I*lift(imaginaryPart z,QQ))});
